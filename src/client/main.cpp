...
bool sendJsonFrame(int clientfd, const json &js)
{
    ...
    while (left > 0)
    {
        ssize_t n = send(clientfd, ptr, left, 0);
        if (n <= 0)
        {
            return false;
        }
...
void ackMessageIfNeeded(int clientfd, const json &js)
{
    if (!js.contains("messageid"))
    {
        return;
    }
...
int main(int argc, char **argv)
{
    ...
    for (;;)
    {
        ...
        case 1: // login业务
        {
            ...
            if (!sendJsonFrame(clientfd, js))
            {
                cerr << "send login msg error:" << js.dump() << endl;
            }
    ...
    else // 登录成功
    {
        ...
        if (responsejs.contains("offlinemsg"))
        {
            ...
            for (string &str : vec)
            {
                ...
                ackMessageIfNeeded(g_clientfd, js);
            }
        }
...
void readTaskHandler(int clientfd)
{
    ...
    for (;;)
    {
        ...
        {
            string frame = recvBuffer.substr(0, pos);
            ...
            {
                cerr << "parse server message error: " << e.what() << " frame=" << frame << endl;
                continue;
            }
            ...
            if (ONE_CHAT_MSG == msgtype)
            {
                cout << js["time"].get<string>() << " [" << js["id"] << "]" << js["name"].get<string>()
                     << " said: " << js["msg"].get<string>() << endl;
                ackMessageIfNeeded(clientfd, js);
                continue;
            }
...
void addgroup(int clientfd, string str)
{
    ...
    if (!sendJsonFrame(clientfd, js))
    {
        cerr << "send addgroup msg error -> " << js.dump() << endl;
    }
}
